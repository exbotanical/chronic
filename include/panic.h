...
#define array_init_or_panic()                                      \
  ({                                                               \
    ...
    if (!__name) {                                                 \
      panic("array_init returned a null pointer - this is a bug"); \
    }                                                              \
    __name;                                                        \
  })
...
#define array_push_or_panic(arr, el)                    \
  if (!array_push(arr, el)) {                           \
    panic("array_push failed - this is a bug, or OOM"); \
  }
...
#define array_get_or_panic(arr, idx)                            \
  ({                                                            \
    ...
    if (!el) {                                                  \
      panic("array_get returned null pointer - this is a bug"); \
    }                                                           \
    el;                                                         \
  })
...
#define ht_init_or_panic(base_capacity, free_value)             \
  ({                                                            \
    ...
    if (!ht) {                                                  \
      panic("ht_init returned a null pointer - this is a bug"); \
    }                                                           \
    ht;                                                         \
  })
...
#define ht_get_or_panic(key, value)                            \
  ({                                                           \
    ...
    if (!v) {                                                  \
      panic("ht_get returned a null pointer - this is a bug"); \
    }                                                          \
    v;                                                         \
  })
...
#define s_copy_or_panic(cp)                                  \
  ({                                                         \
    char* __n = s_copy(cp);                                  \
    if (!__n) {                                              \
      panic("copy returned a null pointer - this is a bug"); \
    }                                                        \
    __n;                                                     \
  })
