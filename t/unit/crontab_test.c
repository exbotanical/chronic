...
static void validate_entry(CronEntry* entry, CronEntry* expected) {
  ...
  is(expected->schedule, entry->schedule, "Expect schedule '%s'",
     expected->schedule);
  is(expected->parent->uname, entry->parent->uname, "Expect parent->uname '%s'",
...
void new_crontab_test(void) {
  ...
  is(ht_get(ct->vars, "PATH"),
     "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
     "has the PATH environment variable");
  is(ht_get(ct->vars, "SHELL"), "/bin/echo",
