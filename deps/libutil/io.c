...
io_write_all_result
io_write_all (FILE *fd, const char *data, size_t *n_write_ptr) {
  ...
  while (total_written < sz) {
    ...
    size_t to_write = (remaining < IO_IO_WRITE_ALL_CHUNK_SZ) ? remaining : IO_IO_WRITE_ALL_CHUNK_SZ;
...
io_read_all_result
io_read_all (FILE *fd, char **data_ptr, size_t *n_read_ptr) {
  char  *data = NULL;
  char  *tmp;
  size_t sz   = 0;
  size_t used = 0;
