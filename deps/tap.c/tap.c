...
vok_at_loc (const char *file, int line, int test, const char *fmt, va_list args) {
  ...
  if (!test) {
    if (*name) {
      diag("  Failed%s test '%s'\n  at %s line %d.", todo_mesg ? " (TODO)" : "", name, file, line);
    } else {
      diag("  Failed%s test at %s line %d.", todo_mesg ? " (TODO)" : "", file, line);
    }
    if (!todo_mesg) {
      failed_tests++;
    }
...
int
is_at_loc (const char *file, int line, const char *got, const char *expected, const char *fmt, ...) {
...
int
isnt_at_loc (const char *file, int line, const char *got, const char *expected, const char *fmt, ...) {
...
int
cmp_ok_at_loc (const char *file, int line, int a, const char *op, int b, const char *fmt, ...) {
  int     test = eq(op, "||")   ? a || b
                 : eq(op, "&&") ? a && b
                 : eq(op, "|")  ? a | b
                 : eq(op, "^")  ? a ^ b
...
exit_status () {
  ...
  if (expected_tests == NO_PLAN) {
    ...
    diag("Looks like you planned %d test%s but ran %d.", expected_tests, expected_tests > 1 ? "s" : "", current_test);
    retval = 255;
  }
  if (failed_tests) {
    diag("Looks like you failed %d test%s of %d run.", failed_tests, failed_tests > 1 ? "s" : "", current_test);
    if (expected_tests == NO_PLAN) {
      retval = failed_tests;
    } else {
...
int
like_at_loc (int for_match, const char *file, int line, const char *got, const char *expected, const char *fmt, ...) {
  int     test;
  regex_t re;
  va_list args;
