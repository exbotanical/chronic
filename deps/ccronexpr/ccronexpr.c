...
#if !defined(_WIN32) && !defined(__AVR__) && !defined(ESP8266) && \
    !defined(ESP_PLATFORM) && !defined(ANDROID) && !defined(TARGET_LIKE_MBED)
...
#error \
    "timegm() is not supported on the ESP platform, please use this library with CRON_USE_LOCAL_TIME"
...
time_t cron_mktime(struct tm* tm) { return cron_mktime_local(tm); }
...
static int last_weekday_of_month(int month, int year) {
  ...
  while (cron_time(&t, &cal)->tm_wday == 6 ||
         cron_time(&t, &cal)->tm_wday == 0) {
...
static int reset_max(struct tm* calendar, int field) {
  ...
  switch (field) {
    case CRON_CF_SECOND:
      calendar->tm_sec = CRON_MAX_SECONDS - 1;
      break;
    case CRON_CF_MINUTE:
      calendar->tm_min = CRON_MAX_MINUTES - 1;
      break;
    case CRON_CF_HOUR_OF_DAY:
      calendar->tm_hour = CRON_MAX_HOURS - 1;
      break;
    case CRON_CF_DAY_OF_WEEK:
    ...
    case CRON_CF_DAY_OF_MONTH:
      calendar->tm_mday =
          last_day_of_month(calendar->tm_mon, calendar->tm_year);
      break;
    case CRON_CF_MONTH:
      calendar->tm_mon = CRON_MAX_MONTHS - 1;
      break;
    case CRON_CF_YEAR:
...
static int reset_all(int (*fn)(struct tm* calendar, int field),
                     struct tm* calendar, uint8_t* fields) {
  int i;
  int res = 0;
 ...
 */
static int find_next_offset(uint8_t* bits, int max, int value, int offset,
                            struct tm* calendar, int field, int nextField,
                            uint8_t* lower_orders, int* res_out) {
  int notfound = 0;
  int err = 0;
...
static int find_next(uint8_t* bits, int max, int value, struct tm* calendar,
                     int field, int nextField, uint8_t* lower_orders,
                     int* res_out) {
  return find_next_offset(bits, max, value, 0, calendar, field, nextField,
                          lower_orders, res_out);
}

static int find_nextprev_day_condition(struct tm* calendar,
                                       uint8_t* days_of_month,
                                       int8_t* day_in_month, int day_of_month,
                                       uint8_t* days_of_week, int day_of_week,
                                       uint8_t* flags, int* changed) {
  static int day;
  if (*changed) {
    if (*flags) {
      if (cron_get_bit(flags, 0))
        ...
        day = closest_weekday(*day_in_month - 1, calendar->tm_mon,
                              calendar->tm_year);
    } else {
      if (*day_in_month < 0)
        day = last_day_of_month(calendar->tm_mon, calendar->tm_year);
  ...
  if (*flags) {
    ...
    if (*day_in_month < 0 &&
        (day_of_month < day + WEEK_DAYS * *day_in_month + 1 ||
         day_of_month >= day + WEEK_DAYS * (*day_in_month + 1) + 1))
      return 1;
    if (*day_in_month > 0 &&
        (day_of_month < WEEK_DAYS * (*day_in_month - 1) + 1 ||
         day_of_month >= WEEK_DAYS * *day_in_month + 1))
      return 1;
  }
  return 0;
}

static int find_nextprev_day(struct tm* calendar, uint8_t* days_of_month,
                             int8_t* day_in_month, int day_of_month,
                             uint8_t* days_of_week, int day_of_week,
                             uint8_t* flags, uint8_t* resets, int* res_out,
                             int offset) {
  int err;
  ...
  while (find_nextprev_day_condition(calendar, days_of_month, day_in_month,
                                     day_of_month, days_of_week, day_of_week,
                                     flags, &changed) &&
         count++ < max) {
...
static int find_next_day(struct tm* calendar, uint8_t* days_of_month,
                         int8_t* day_in_month, int day_of_month,
                         uint8_t* days_of_week, int day_of_week, uint8_t* flags,
                         uint8_t* resets, int* res_out) {
  return find_nextprev_day(calendar, days_of_month, day_in_month, day_of_month,
                           days_of_week, day_of_week, flags, resets, res_out,
                           1);
}

static int do_nextprev(
    int (*find)(uint8_t* bits, int max, int value, struct tm* calendar,
                int field, int nextField, uint8_t* lower_orders, int* res_out),
    int (*find_day)(struct tm* calendar, uint8_t* days_of_month,
                    int8_t* day_in_month, int day_of_month,
                    uint8_t* days_of_week, int day_of_week, uint8_t* flags,
                    uint8_t* resets, int* res_out),
    int (*find_offset)(uint8_t* bits, int max, int value, int offset,
                       struct tm* calendar, int field, int nextField,
                       uint8_t* lower_orders, int* res_out),
  ...
  for (;;) {
    *resets = 0;
    ...
    update_value =
        find(expr->seconds, CRON_MAX_SECONDS + CRON_MAX_LEAP_SECONDS, value,
             calendar, CRON_CF_SECOND, CRON_CF_MINUTE, empty_list, &res);
    if (0 != res) break;
    ...
    update_value = find(expr->minutes, CRON_MAX_MINUTES, value, calendar,
    ...
    update_value = find(expr->hours, CRON_MAX_HOURS, value, calendar,
    ...
    update_value = find_day(calendar, expr->days_of_month, expr->day_in_month,
                            value, expr->days_of_week, calendar->tm_wday,
                            expr->flags, resets, &res);
    if (0 != res) break;
    if (value == update_value)
    ...
    value =
        calendar
            ->tm_mon; /*day already adds one if no day in same value is found*/
    update_value = find(expr->months, CRON_MAX_MONTHS, value, calendar,
    ...
    update_value = find_offset(expr->years, CRON_MAX_YEARS - CRON_MIN_YEARS,
                               value, YEAR_OFFSET - CRON_MIN_YEARS, calendar,
                               CRON_CF_YEAR, CRON_CF_NEXT, resets, &res);
    if (0 != res) break;
...
static int do_next(cron_expr* expr, struct tm* calendar, int dot) {
  return do_nextprev(find_next, find_next_day, find_next_offset, expr, calendar,
                     dot);
}
...
static char* str_replace(char* orig, const char* rep, const char* with) {
   ...
   */
  tmp = result =
      (char*)cron_malloc(strlen(orig) + (len_with - len_rep) * count + 1);
...
static char* replace_ordinals(char* value, const char* const* arr,
                              size_t arr_len) {
  size_t i;
  char* cur = value;
  char* res = NULL;
...
static void get_range(const char* field, int min, int max, int* res,
                      const char** error) {
  char** parts = NULL;
  size_t len = 0;
  int err = 0;
...
static void set_number_hits_offset(const char* value, uint8_t* target, int min,
                                   int max, int offset, const char** error) {
  size_t i = 0;
  int i1 = 0;
  size_t len = 0;
  int range[2];
...
static void set_number_hits(const char* value, uint8_t* target, int min,
                            int max, const char** error) {
...
static void set_days_of_week(char* field, uint8_t* days_of_week,
                             int8_t* day_in_month, const char** error) {
  const int max = 7;
  char* replaced = NULL;
  size_t pos;
  int err;
...
static int set_days_of_month(char* field, uint8_t* days_of_month,
                             uint8_t* days_of_week, int8_t* day_in_month,
                             uint8_t* flags, const char** error) {
  int i1;
  int ret = 0;
  size_t pos;
  int err;
...
void cron_parse_expr(const char* expression, cron_expr* target,
  ...
  *error = NULL;
  ...
  if (len > 5) {
    if (fields[pos][0] == 'L') {
      field = fields[pos++];
      set_number_hits(field + 1, target->seconds, 0,
                      CRON_MAX_SECONDS + CRON_MAX_LEAP_SECONDS, error);
    } else {
      set_number_hits(fields[pos++], target->seconds, 0, CRON_MAX_SECONDS,
                      error);
    }
  ...
  if (*error) goto return_res;
  ret = set_days_of_month(fields[pos++], target->days_of_month,
  ...
  if (*error) goto return_res;
  if (0 == ret) {
    set_days_of_week(fields[pos++], target->days_of_week, target->day_in_month,
                     error);
    if (*error) goto return_res;
  }
  if (len < 7) {
...
time_t cron_nextprev(int (*fn)(cron_expr* expr, struct tm* calendar, int dot),
 ...
 * and reset the calendar.
 */
static int find_prev_offset(uint8_t* bits, int max, int value, int offset,
                            struct tm* calendar, int field, int nextField,
                            uint8_t* lower_orders, int* res_out) {
  int notfound = 0;
  int err = 0;
...
static int find_prev(uint8_t* bits, int max, int value, struct tm* calendar,
                     int field, int nextField, uint8_t* lower_orders,
                     int* res_out) {
  return find_prev_offset(bits, max, value, 0, calendar, field, nextField,
                          lower_orders, res_out);
}

static int find_prev_day(struct tm* calendar, uint8_t* days_of_month,
                         int8_t* day_in_month, int day_of_month,
                         uint8_t* days_of_week, int day_of_week, uint8_t* flags,
                         uint8_t* resets, int* res_out) {
  return find_nextprev_day(calendar, days_of_month, day_in_month, day_of_month,
...
static int do_prev(cron_expr* expr, struct tm* calendar, int dot) {
  return do_nextprev(find_prev, find_prev_day, find_prev_offset, expr, calendar,
                     dot);
}
