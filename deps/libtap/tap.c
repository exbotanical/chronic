...
static unsigned int
__tap_vok (unsigned int ok, const char* fn_name, const char* file, const unsigned int line, const char* fmt, va_list args) {
...
unsigned int
__tap_ok (unsigned int ok, const char* fn_name, const char* file, const unsigned int line, const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
__tap_match (
  ...
  if (!re) {
    tap_panic(TAP_FAILURE_EXIT_STATUS, "Failed to compile regex pattern: %s (reason: %s)\n", error, pattern);
  }
  ...
  va_list args;
  va_start(args, fmt);
  unsigned int ret = __tap_vok(want_match ? rc >= 0 : rc < 0, fn_name, file, line, fmt, args);
  va_end(args);
  return ret;
}
